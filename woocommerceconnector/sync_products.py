...
def make_item(warehouse, woocommerce_item, woocommerce_item_list):
    if has_variants(woocommerce_item):
        ...
        woocommerce_item["variants"] = get_woocommerce_item_variants(
            woocommerce_item.get("id")
        )
        ...
        create_item(
            ...
            woocommerce_item_list=woocommerce_item_list,
        )
        create_item_variants(
            woocommerce_item,
            warehouse,
            attributes,
            woocommerce_variants_attr_list,
            woocommerce_item_list,
        )
    else:
...
def create_item(
    ...
    item_dict = {
        ...
        "description": woocommerce_item.get("description")
        or woocommerce_item.get("name"),
        "woocommerce_description": woocommerce_item.get("description")
        or woocommerce_item.get("name"),
        "woocommerce_short_description": woocommerce_item.get("short_description")
        or woocommerce_item.get("name"),
        "item_group": get_item_group(woocommerce_item.get("categories")),
        "has_variants": has_variant,
        "attributes": attributes or [],
        "stock_uom": get_erpnext_uom(
            woocommerce_item, woocommerce_settings, attributes
        ),
        "stock_keeping_unit": woocommerce_item.get(
            "sku"
        ),  # or get_sku(woocommerce_item),
        ...
        "web_long_description": woocommerce_item.get("description")
        or woocommerce_item.get("name")
    ...
    if template_item:
        ...
        # single & templates
        item_dict["product_category"] = get_categories(
            woocommerce_item, is_variant=False
        )
    ...
    if len(attributes) > 0:
        for attr in attributes:
            if attr["attribute"] == woocommerce_settings.attribute_for_uom:
                uom_match = frappe.get_all(
                    "UOM",
                    filters={"uom_name": "{0}".format(attr["attribute_value"])},
                    fields=["name"],
                )
                if len(uom_match) > 0:
                    ...
                    new_uom = frappe.get_doc(
                        {"doctype": "UOM", "uom_name": attr["attribute_value"]}
                    ).insert()
                    return attr["attribute_value"]
    else:
        return "Nos"
...
def create_item_variants(
    ...
    template_item = frappe.db.get_value(
        "Item",
        filters={"woocommerce_product_id": woocommerce_item.get("id")},
            ...
            for variant in woocommerce_variants_attr_list:
                for attr in attributes:
                    if attr["attribute"] == variant["name"]:
                        attr["attribute_value"] = get_attribute_value(
                            variant.get("option"), variant
                        )
                        break
            create_item(
                woocommerce_item_variant,
...
# fix this
def is_item_exists(
    item_dict, attributes=None, variant_of=None, woocommerce_item_list=[]
):
    woocommerce_item_list.append(cstr(item_dict.get("woocommerce_product_id")))

    erp_item_match = frappe.get_all(
        "Item",
...
def create_attribute(woocommerce_item):
    ...
    for attr in woocommerce_item.get("attributes"):
        if not frappe.db.get_value("Item Attribute", attr.get("name"), "name"):
            ...
            attribute.append(
                {"attribute": attr.get("name"), "attribute_value": attribute_value}
            )
        else:
            ...
            if not item_attr.numeric_values:
                ...
                if (
                    len(item_attr.item_attribute_values) > old_len
                ):  # only save item attributes if they have changed
                    item_attr.save()
            if len(attr.get("options")[0]) > 140:
                attribute_value = attr.get("options")[0][:140]
            else:
...
def set_new_attribute_values(item_attr, values):
    for attr_value in values:
        if not any(
            ...
            item_attr.append(
                "item_attribute_values",
                {"attribute_value": attr_value[:140], "abbr": attr_value[:140]},
            )
    return item_attr
...
def get_attribute_value(variant_attr_val, attribute):
    attribute_value = frappe.db.sql(
        ...
        (attribute["name"], variant_attr_val, variant_attr_val),
        as_list=1,
    )
    # check if this is really not available due to numeric value or if it is from changing attributes after creation
    if len(attribute_value) == 0 and (
        "{0}".format(cint(variant_attr_val)) != "{0}".format(variant_attr_val)
    ):
        frappe.log_error(
            "Attribute value mismatch: {attr}: {value} (potential change in attribute?".format(
                attr=attribute, value=variant_attr_val
            ),
            "WooCommerce Attribute Mismatch",
        )
...
def add_to_price_list(item, name):
    ...
    item_price_name = frappe.db.get_value(
        "Item Price", {"item_code": name, "price_list": price_list}, "name"
    )
    ...
    for item in get_erpnext_items(price_list):
        try:
            sync_item_with_woocommerce(
                ...
                price_list,
                warehouse,
                woocommerce_item_list.get(item.get("woocommerce_product_id")),
            )
...
def get_erpnext_items(price_list):
    ...
    if woocommerce_settings.last_sync_datetime:
        last_sync_condition = "and modified >= '{0}' ".format(
            woocommerce_settings.last_sync_datetime
        )
        item_price_condition = "AND `tabItem Price`.`modified` >= '{0}' ".format(
            woocommerce_settings.last_sync_datetime
        )
    ...
    return [
        frappe._dict(tupleized)
        for tupleized in set(
            tuple(item.items()) for item in erpnext_items + updated_price_item_list
        )
    ]
...
def sync_item_with_woocommerce(item, price_list, warehouse, woocommerce_item=None):
    ...
    if item.get("has_variants"):  # we are dealing a variable product
        ...
        variant_list, options, variant_item_name = get_variant_attributes(
    ...
    if not item.get("woocommerce_product_id"):
        ...
        try:
            ...
            if e.args[0] and (
                e.args[0].startswith("404") or e.args[0].startswith("400")
            ):
    ...
    if erp_item.image:
        ...
        img_details = frappe.db.get_value(
            "File", {"file_url": erp_item.image}, ["modified"]
        )

        if not item_image or datetime.datetime(
            item_image.date_modified, "%Y-%m-%dT%H:%M:%S"
        ) < datetime.datetime(img_details[0], "%Y-%m-%d %H:%M:%S.%f"):
...
def sync_item_image(item):
    ...
    if item.image:
        ...
        image_info["images"][0]["src"] = (
            "https://" + cstr(frappe.local.site) + img_details[1]
        )
...
def item_image_exists(woocommerce_product_id, image_info):
    ...
    for image in get_woocommerce_item_image(woocommerce_product_id):
        if image_info.get("image").get("filename"):
            if (
                os.path.splitext(image.get("src"))[0].split("/")[-1]
                == os.path.splitext(image_info.get("image").get("filename"))[0]
            ):
                return True
        elif image_info.get("image").get("src"):
            if (
                os.path.splitext(image.get("src"))[0].split("/")[-1]
                == os.path.splitext(image_info.get("image").get("src"))[0].split("/")[
                    -1
                ]
            ):
                return True
...
def get_variant_attributes(item, price_list, warehouse):
    ...
    for i, variant in enumerate(
        frappe.get_all(
            "Item", filters={"variant_of": item.get("name")}, fields=["name"]
        )
    ):
...
def get_price_and_stock_details(item, warehouse, price_list):
    ...
    if item.weight_per_unit:
        if item.weight_uom and item.weight_uom.lower() in [
            "kg",
            "g",
            "oz",
            "lb",
            ...
            item_price_and_quantity.update(
                {
                    "weight": "{0}".format(
                        get_weight_in_woocommerce_unit(
    ...
    if item.stock_keeping_unit:
        item_price_and_quantity = {"sku": "{0}".format(item.stock_keeping_unit)}
...
def get_weight_in_grams(weight, weight_uom):
    convert_to_gram = {"kg": 1000, "lb": 453.592, "oz": 28.3495, "g": 1}
...
def trigger_update_item_stock(doc, method):
    if doc.flags.via_stock_ledger_entry:
        woocommerce_settings = frappe.get_doc(
            "WooCommerce Config", "WooCommerce Config"
        )
        if (
            woocommerce_settings.woocommerce_url
            and woocommerce_settings.enable_woocommerce
            and woocommerce_settings.trigger_update_item_stock
        ):
...
def update_item_stock(item_code, woocommerce_settings, bin=None):
    ...
    if item.sync_qty_with_woocommerce:
        if not item.woocommerce_product_id:
            make_woocommerce_log(
                title="WooCommerce ID missing",
                status="Error",
                method="sync_woocommerce_items",
                message="Please sync WooCommerce IDs to ERP (missing for item {0})".format(
                    item_code
                ),
                request_data=item_code,
                exception=True,
            )
        else:
            ...
            bin_since_last_sync = frappe.db.sql(
                """SELECT COUNT(`name`) FROM `tabBin` WHERE `item_code` = '{item_code}' AND `modified` > '{last_sync_datetime}'""".format(
                    item_code=item_code, last_sync_datetime=last_sync_datetime
                ),
                as_list=True,
            )[0][0]
            if bin_since_last_sync > 0:
                bin = get_bin(item_code, woocommerce_settings.warehouse)
                qty = bin.actual_qty - bin.reserved_qty
...
def get_product_update_dict_and_resource(
    woocommerce_product_id, woocommerce_variant_id=None, is_variant=False, actual_qty=0
):
    item_data = {}
    ...
    if is_variant:
        resource = "products/{0}/variations/{1}".format(
...
def add_w_id_to_erp():
    ...
    make_woocommerce_log(
        title="Syncing IDs",
        status="Started",
        method="add_w_id_to_erp",
        message="Item: {0}".format(woo_items),
        request_data={},
        exception=True,
    )
    for woocommerce_item in woo_items:
        update_item = """UPDATE `tabItem`
            ...
            WHERE `barcode` = '{1}';""".format(
            woocommerce_item.get("id"), woocommerce_item.get("sku")
        )
        frappe.db.sql(update_item)
        frappe.db.commit()
        for woocommerce_variant in get_woocommerce_item_variants(
            woocommerce_item.get("id")
        ):
            update_variant = """UPDATE `tabItem`
                SET `woocommerce_variant_id` = '{0}', `woocommerce_product_id` = '{1}'
                WHERE `barcode` = '{2}';""".format(
                woocommerce_variant.get("id"),
                woocommerce_item.get("id"),
                woocommerce_variant.get("sku"),
            )
    ...
    if len(different_unit_items) > 0:
        ...
        for i in different_unit_items:
            ...
            try:
                ...
                make_woocommerce_log(
                    title="WC rewrite stock uom",
                    status="Error",
                    method="rewrite_stock_uom_from_wc_unit",
                    message="Unabe to rewrite stock of item {0} from {1} to {2}: {3}".format(
                        i.item_code, i.stock_uom, i.unit, err
                    ),
                    request_data=None,
                    exception=True,
                )
